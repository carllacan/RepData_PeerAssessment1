...
So there are `r length(NAvalues) ` rows with an NA value intheir steps field. We will fix this by creating a new dataset with the NA values replaced with the number of steps taken in the corresponding interval averaged across all days.
...
Compare those results with the ones we obtained before (mean: `r mean(dailysteps)`, median:`r median(dailysteps)`). Interestingly, the mean and the median have equal values now, which could be due to the reduced number of outliers (which skew the mean but not the media) in the second dataset. 
